...
/// <summary>
/// IntakeAgent: Handles intent detection and policy gating.
/// Analyzes user queries to determine intent and applies policy-based routing rules.
/// </summary>
public class IntakeAgent : IAgentService
{
    ...
    public string AgentName => "Intake Agent";

    public IntakeAgent(PersistentAgentsClient agentsClient, string modelDeploymentName, ILogger<IntakeAgent> logger)
    {
        _agentsClient = agentsClient;
        _modelDeploymentName = modelDeploymentName;
        _logger = logger;
        
        _logger.LogInformation("IntakeAgent initialized with model: {ModelName}", modelDeploymentName);
    }
    ...
    private string GetOrResolveAgentId()
    {
        ...
        var systemPrompt = @"You are an Intake Agent responsible for analyzing user queries and determining intent.
Your role is to:
1. Understand the user's question and intent
2. Classify the query type (policy question, SOP question, general question, etc.)
3. Identify any policy constraints or gating requirements
4. Extract key entities and topics from the query
...
Respond with a JSON object containing:
{
  ...
  ""requires_policy_check"": true|false,
  ""reasoning"": ""Brief explanation of the analysis""
}";

        const string agentName = "Intake Agent";
    ...
    public async Task<string> ProcessQueryAsync(string query, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("IntakeAgent processing query for intent analysis: {Query}", 
                query.Length > 100 ? query.Substring(0, 100) + "..." : query);
            
            var agentId = GetOrResolveAgentId();

            // Create a new thread for each intake analysis (stateless)
            _logger.LogDebug("Creating new thread for intake analysis");
            ...
            {
                _logger.LogInformation("Successfully generated intent analysis (length: {Length} characters)", textContent.Text.Length);
                return textContent.Text;
            }

            _logger.LogWarning("No assistant message found in thread: {ThreadId}", threadId);
            return "I apologize, but I couldn't analyze the intent at this time.";
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error processing intent analysis: {Message}", ex.Message);
            return $"Error processing intent analysis: {ex.Message}";
        }
    }
}
